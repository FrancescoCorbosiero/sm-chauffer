  ...
  return (
    <article className="group flex flex-col bg-white border border-[var(--color-border)] rounded-xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-[var(--shadow-lg)]">
      <div className="relative aspect-[5/4] w-full overflow-hidden bg-[var(--color-surface-2)]">
        ...
        />
        {v.badge && (
          <span className="absolute top-4 left-4 bg-white text-[var(--color-ink)] text-[10px] uppercase tracking-[0.22em] px-3 py-1.5 rounded-full font-medium border border-[var(--color-border)]">
            {v.badge}
          </span>
        )}
      </div>
        ...
        <div className="flex items-center justify-between gap-4 pt-5 border-t border-[var(--color-divider)]">
          <div>
            ...
            <div className="text-[var(--color-ink)] text-lg font-medium">
              ...
              <span className="text-xs text-[var(--color-text-muted)] font-normal">{t.fleetCard.perHour}</span>
            </div>
          </div>
          <Link
            href={`/contact?car=${vehicle.id}`}
            className="inline-flex items-center gap-1.5 text-xs uppercase tracking-[0.2em] text-[var(--color-ink)] hover:text-[var(--color-text-muted)] transition-colors"
          >
            {t.fleetCard.book} <ArrowUpRight size={14} />
          </Link>
        </div>
      </div>
    </article>
